...
export default function Login() {
  ...
  const navigateAfterLogin = async (session) => {
    ...
    navigate(profile?.role === 'admin' ? createPageUrl('AdminLicenses') : '/', { replace: true });
  };
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) navigateAfterLogin(data.session);
    });
  }, [navigate]);
  const submit = async (event) => {
    ...
    if (data.session) await navigateAfterLogin(data.session);
  };
